...
drupal_add_css(path_to_theme().'/elephant-parents.css');
drupal_add_js(path_to_theme().'/elephant-parents.js');
...
function phptemplate_preprocess(&$variables, $hook) 
{
	...
	if($hook == 'node' || $hook == 'page' && $variables['node']->type == 'question') {
		$variables['title'] = 'Question: '. $variables['title'];
	}
}
...
function phptemplate_box($title = '', $content = '', $region = 'main') {
	// hide the comment form on the node page for non-admins
